...
namespace AMR_Server.Infrastructure.Identity
{
    public class IdentityService : IIdentityService
    {
        ...
        public async Task<User> Register(string username, string password)
        {
            ...
            {
                user.UserId = await CreateUserBasicDataAsync(result.UserId, username);//Insert row in UserBasicData table (Details data about user)
                user.Token = GenerateToken(result.UserId);//Generate Token if Usser has been created
                return user;
            }
            return null;
        }
        ...
        public async Task<User> AuthenticateAD(string userName, string password)
        {
        ...
        private async Task<User> GetUserAD(UserPrincipal foundUser, string password)
        {
            ...
            if (foundUser != null)
            {
                ...
                if (user == null)
                {
                    ...
                    userDto.UserId = await CreateUserBasicDataAsync(result.UserId, foundUser.SamAccountName); //Insert row in UserBasicData table
                }
                var userBasicData = _context.UserBasicData.SingleOrDefault(x => x.UserName == foundUser.SamAccountName);
                if (userBasicData != null)
                {
                    userDto.UserId = userBasicData.UserId;
                }
                userDto.Token = GenerateToken(foundUser.Guid.ToString());
                return userDto;
            }
            return null;
        }
